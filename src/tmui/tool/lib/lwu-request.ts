 ...
 * - mask - 是否显示透明蒙层
 */
export const loading = (params: { title: string; mask?: boolean }) => {
  uni.showLoading({
    title: params.title,
 ...
 * @returns {void}
 */
export const msg = (params: { title: string; duration?: number }) => {
  uni.showToast({
    title: params.title,
    icon: 'none',
    image: '',
  ...
  private interceptor(url: string, before: Function | undefined, after: Function | undefined) {
    uni.addInterceptor('request', {
      invoke: (args) => {
        ...
        if (args.method === 'GET') {
          args.data =
            this.config.buildQueryString && this.config.buildQueryString(args.data)
