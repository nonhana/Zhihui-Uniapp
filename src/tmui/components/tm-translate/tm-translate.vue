<template>
  ...
  >
    <!-- #ifdef APP-NVUE -->
    <view v-if="isLoadEl" ref="nvueElAni" :animation="animationData" :class="['flex-col flex ', animationName + reverseAniPrefxname, customClass]">
    ...
    tmid.value = setTimeout(function () {
      animation.transition(
        el,
        {
          ...
          timingFunction: 'cubic-bezier(.04,.78,.42,1)',
          delay: 0, //ms
        },
        () => {
          emits('end')
