...
export function rangeTimeArray(
  dateStr: string | number | Date | dayjs.Dayjs,
...
export function getIndexNowbydate(tmArray: timeArrayType, nowtime: dayjs.Dayjs, detail: showDetail) {
  ...
  return [...idx, ...[0, 0, 0, 0, 0, 0]].slice(0, 6)
}
/**
 * 根据现有索引值返回当前时间。
 * @param tmArray
 ...
 * @param end
 */
export function checkNowDateisBetween(
  nowdate: string | number | Date | dayjs.Dayjs,
  start: string | number | Date | dayjs.Dayjs,
  end: string | number | Date | dayjs.Dayjs
) {
