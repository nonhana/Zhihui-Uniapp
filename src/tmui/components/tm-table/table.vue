        ...
        <tm-row v-if="props.showHeader" :transprent="true" :width="totalTableWidth" :column="_data.header.length">
          ...
          >
            <view
              ...
              :style="{
                width: (item.opts?.sort ?? false ? totalTableWidth / _data.header.length - 60 : totalTableWidth / _data.header.length - 60) + 'rpx',
              }"
            >
              <tm-text
        ...
        <view v-for="(item, index) in _maxrows" :key="item" :style="{ width: totalTableWidth + 'rpx' }">
          ...
          >
            <tm-col
              ...
              _class="flex flex-row flex-row-center-center"
            >
              <view :userInteractionEnabled="false" class="flex-1" v-if="((_rows[index2] ?? [])[index].opts?.type ?? 'text') == 'button'">
                <tm-button
