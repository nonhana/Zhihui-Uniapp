  ...
  >
    ...
    <view class="flex-1 flex-row flex-row-center-between absolute" :style="{ width: _width, top: nodeSize / 2 + 'rpx' }">
      <view :style="{ width: _widthLine }">
        <tm-divider :border="2" v-if="_cureent > 0" :color="_color" :followDark="props.followDark" :dark="props.dark" :margin="[0, 0]"></tm-divider>
      </view>
      <view :style="{ width: _widthLine }">
        <tm-divider
          :border="2"
    ...
    <view @click.stop="stepClick" class="flex flex-col relative" style="justify-content: flex-start; align-items: center">
      <tm-sheet
        ...
        :height="nodeSize"
      >
        <tm-text v-if="!_icon && !status && _typeModel != 'dot'" :userInteractionEnabled="false" :font-size="22" :label="_cureent + 1"></tm-text>
